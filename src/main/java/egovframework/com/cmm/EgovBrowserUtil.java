...
public class EgovBrowserUtil {
	...
	public static HashMap<String,String> getBrowser(String userAgent) {
		...
		pattern = Pattern.compile("KAKAOTALK ([0-9]{1,3}.[0-9]{1,3})");
		matcher = pattern.matcher(userAgent);
		if (matcher.find())
		{
		    result.put(TYPEKEY,KAKAOTALK);
		    result.put(VERSIONKEY,matcher.group(1));
			return result;		    
		}
	...
	public static String getDisposition(String filename, String userAgent, String charSet) throws Exception {
		...
		return getDisposition("attachment;", filename, userAgent, charSet);
	}
	
	public static String getDisposition(String disposition, String filename, String userAgent, String charSet) throws Exception {
		...
		if ( EgovBrowserUtil.MSIE.equals(result.get(EgovBrowserUtil.TYPEKEY)) && version <= 8.0f ) {
			encodedFilename = "Content-Disposition: " + disposition + " filename="+URLEncoder.encode(filename, charSet).replaceAll("\\+", "%20");
		} /* else if ( EgovBrowserUtil.OTHER.equals(result.get(EgovBrowserUtil.TYPEKEY)) ) {
			throw new RuntimeException("Not supported browser");
			}*/ else {
			encodedFilename = disposition + " filename*="+charSet+"''"+URLEncoder.encode(filename, charSet);
		}
